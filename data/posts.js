...
let exportedMethods = {
    ...
    async deletePost(id) {
        ...
        if (deletionInfo.lastErrorObject.n === 0) throw "Error: Could not delete post";
    ...
    async claimPost(id) {
        ...
        const postCollection = await posts();
        const updateInfo = await postCollection.updateOne({ _id: new ObjectId(id) }, { $set: { claimed: true } });
